        ...
        {ranking && ranking.length > 0 ? (
          <div className="divide-y divide-slate-700">
            {ranking.map((r: any) => {
              ...
              return (
                <div key={r.user_id} className={`flex items-center gap-4 px-5 py-4 transition-colors ${isMe ? 'bg-indigo-600/20' : 'hover:bg-slate-700/50'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`font-semibold truncate ${isMe ? 'text-indigo-300' : 'text-white'}`}>
                      {r.nombre} {r.apellido} {isMe && '(vos)'}
                    </p>
                    <p className="text-xs text-slate-500">{r.predictions_count} pronósticos · {r.exact_results} exactos</p>
                  </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-400">Nadie en el ranking todavía. ¡Cargá tus pronósticos!</p>
          </div>
        )}
      </div>
    </div>
