    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-green-600/20 border border-green-500/30 rounded-2xl p-4 text-center">
          <p className="text-2xl font-bold text-green-400">{finished.filter((p: any) => p.points_earned === 3).length}</p>
        ...
        <div className="bg-blue-600/20 border border-blue-500/30 rounded-2xl p-4 text-center">
          ...
          <p className="text-xs text-slate-400">Pronosticos</p>
        </div>
      </div>
      {predictions.length > 0 ? (
            ...
            <div key={p.id} className="bg-slate-800 border border-slate-700 rounded-2xl p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                {p.points_earned !== null && (
                  <span className={`text-sm font-bold px-2 py-0.5 rounded-full ${p.points_earned === 3 ? 'bg-green-500/20 text-green-400' : p.points_earned > 0 ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-700 text-slate-400'}`}>
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <div className="text-center">
                  <p className="text-slate-400 text-xs">Tu pronos.</p>
                  <p className="text-white font-bold">{p.predicted_home} - {p.predicted_away}</p>
                </div>
        ...
        <div className="text-center py-12">
          ...
          <p className="text-slate-400">No hay pronosticos todavia</p>
        </div>
      )}
    </div>
  )
